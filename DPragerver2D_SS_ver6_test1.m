...
% DF_DSIG,DEP are updated for next iteration, and dEp for next iteration is calculated based on new DEP
...
dEps11 = zeros(iteration,1);
dEps22 = zeros(iteration,1);
dEps44 = zeros(iteration,1);
...
%*******Initialization**************%
  ...
  % Calculate initial parameters
    ...
    [DF_DSIG_t,M_EF_DF_DSIG_t,DEP_t,TERM1_t,TERM2_t,DP_t] = DELASTOPLASTIC(P_trial(a,1),Q_trial(a,1),S_trial,DE,Mu(a,1));
   ...
   if YIELD_trial(a-1,1) > -0.00001
   [DF_DSIG,M_EF_DF_DSIG,DEP_f,TERM1,TERM2,DP] = DELASTOPLASTIC(P(a,1),Q(a,1),S,DE,Mu(a,1));
   YIELD_final(a,1) = Q(a,1)-Mu(a,1)*P(a,1);
   
    dEps =  [  -DEP_f(1,4)*dEps12/DEP_f(1,1);
	0;
	0;
	dEps12];
    end
       ...
       % storing stress result
          ...
          dEps11(a,1)= DSTRAN_P(1,1);
          dEps22(a,1)= DSTRAN_P(2,1);
          dEps33(a,1)= DSTRAN_P(3,1);
          dEps44(a,1)= DSTRAN_P(4,1);
   
          
          shear_ratio(a,1) = S12(a,1)/S11(a,1);
          stress_rotation(a,1) = 0.5*atan(2*S12(a,1)/abs(S22(a,1)-S11(a,1)))*180/3.14;
          strain_rotation(a,1) = 0.5*atan(dEps44(a,1)/abs(dEps22(a,1)-dEps11(a,1)))*180/3.14; % for coaxial stress and strain rotation should coincide
	end
